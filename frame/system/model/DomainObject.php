<?php
	/*
	 * Наследники должны определить: переменные $table (таблица БД в которой хранится информация),$modelName (фактически префикс, который стоит перед идентификатором в таблице)
	 * функции insertObject() - вставка нового объекта в БД
	...
	abstract class DomainObject{
		...
		protected abstract function updateObject();//функция для изменнеия
		public function loadObject(){
			//загрузка в базу данных
			//если объект новый то запустит инсерт, если изменялся объект из БД то апдейт			
			if ($this->id==0){
				$this->insertObject();
				$this->setId($this->db->getDBH()->lastInsertId());
			}
			else{				
				$this->updateObject();
			}
		...
		public function getObject($id){
			if (isset($id)&&is_numeric($id)&&$this->db->getNumRow($this->table,array($this->modelName."_id"=>$id))>0){
		...
		public static function getCollection($db,$table,$className,$data=false,$modelName="",$order=false){
			if ($modelName==""){//обычно имя модели должно совпадать с именем таблицы
				$modelName=$table;			
			}			
			$data=$db->getAllRow($table,$data,$order);	
			if (count($data)>0){
