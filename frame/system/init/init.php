<?php
	...
	class Init{
		...
		private function __construct($siteRoot){
			...
			$this->config=Config::getConfig($siteRoot);//получаем конфиги						
			...
			if ($smarty){	
				...
				if ($url->header&&$url->footer){
					$this->command->setHeaderAndFooter($_SERVER['DOCUMENT_ROOT'].$this->siteRoot."/application/view/".$url->header.".tpl",$_SERVER['DOCUMENT_ROOT'].$this->siteRoot."/application/view/".$url->footer.".tpl");
				}
				else if (!($url->onlyMainView==1)){//задаЄм стандартные header и footer
		...
		public static function init(){
			//примен€ем “ќЋ№ ќ после инициализации
			return self::$instance;
		}
